...
class Command(BaseCommand):
    ...
    def add_arguments(self , parser):
        parser.add_argument("-p", "--provincia", type=str)
    ...
    def handle(self, *args, **options):
        print('Updating...')
        provinceName = options["provincia"] if options["provincia"] else "Andalucía"
        start = datetime.now() - timedelta(days=14)
        start_dt = start.date()
        end_dt = datetime.now().date()
        for dt in self.daterange(start_dt, end_dt):
            if (provinceName=="Andalucía"):
                self.getAcumulatedTodayReg(dt)
            else:
                self.getAcumulatedTodayProv(provinceName,dt)
